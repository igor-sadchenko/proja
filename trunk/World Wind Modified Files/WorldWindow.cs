...
//		protected DownloadIndicator m_downloadIndicator;
        ...
        {
            //MessageBox.Show("Button pressed:" + e.Button.ToString() + "\nNumber of clicks:" + e.Clicks.ToString()
...
*/				
        ...
        #region Touching Region

        Dictionary<int, Touch> mTouches = new Dictionary<int, Touch>(); 
        double mDistanceBetweenTwoFingers;
        int prevID1=-1, prevID2=-1;
        int prevTiltID1 = -1, prevTiltID2 = -1; 

        public void OnTouchDown(object sender, TouchEventArgs e)
        {
            mTouches.Add(e.ID , new Touch(e.ID, e.point)); 
            if (mTouches.Count == 1)
            {
            ...
            {
                int i = 0;
                Touch f1 = null, f2 = null;
                foreach (Touch f in mTouches.Values)
                {
                    if (i == 0)
                        f1 = f;
                    else
                ...
                mDistanceBetweenTwoFingers = Touch.GetDistance(f1.mPosition, f2.mPosition);
            }
        }

        public void OnTouchMove(object sender, TouchEventArgs e)
        {
            mTouches[e.ID].Move(e.point);
            ...
            else if (mTouches.Count == 2)
            {
                ...
                if ((f1.mID == prevID1 || f1.mID == prevID2) && (f2.mID == prevID1 || f2.mID == prevID2))
                {
                    //Zooming
                    double distance = Touch.GetDistance(f1.mPosition, f2.mPosition);
                    Point midPoint = Touch.GetMidPoint(f1.mPosition, f2.mPosition);

                    OnMouseWheel(new MouseEventArgs(MouseButtons.None, 0, midPoint.X, midPoint.Y, ((int)(distance - mDistanceBetweenTwoFingers) * 15)));
                    mDistanceBetweenTwoFingers = distance;

                    //Rotation
                    Touch movedFinger = mTouches[e.ID]; 
                    Touch staticFinger = null ;
                    if (f1.mID == e.ID)
                        staticFinger = f2;
                    else
                        staticFinger = f1;

                    double angle = GetAngle(staticFinger.mPosition, movedFinger.mOldPosition,
                                            staticFinger.mPosition, movedFinger.mPosition);

                    Angle rotate = Angle.FromDegrees(angle);
                    this.drawArgs.WorldCamera.Heading += rotate;
                    this.drawArgs.WorldCamera.RotationYawPitchRoll(Angle.Zero, Angle.Zero, rotate);
                }
                prevID1 = f1.mID;
                prevID2 = f2.mID;
            }
            else if (mTouches.Count == 3)
            {
                int i = 0;
                Touch f1 = null, f2 = null, f3 = null;
                foreach (Touch f in mTouches.Values)
                {
                    if (i == 0)
                        f1 = f;
                ...
                if ((f1.mID == prevTiltID1|| f1.mID == prevTiltID2) && (f2.mID == prevTiltID1|| f2.mID == prevTiltID2))
                {
                    if (f3.mOldPosition != Point.Empty)
                    {
                        double distance = Touch.GetDistance(f3.mPosition, f3.mOldPosition);

                        if( f3.mPosition.Y > f3.mOldPosition.Y )
                            this.drawArgs.WorldCamera.Tilt += Angle.FromDegrees(distance);
                        else
                            this.drawArgs.WorldCamera.Tilt += Angle.FromDegrees(-distance);
        ...
        public double GetAngle(Point p1, Point p2 , Point p3 , Point p4)
        {
            ...
            if (p1.X == p2.X)
                ...
                m1 = (float)(p1.Y - p2.Y) / (p1.X - p2.X);

            if (p3.X == p4.X)
                yAxis2 = true;
            else
                m2 = (float)(p3.Y - p4.Y) / (p3.X - p4.X);
