...
bool BaseApplication::processUnbufferedKeyInput(const FrameEvent& evt)
{
	...
	if (mKeyboard->isKeyDown(KC_DOWN) )
		mSceneMgr->getRootSceneNode()->pitch(mRotScale);

	if(mKeyboard->isKeyDown(KC_UP))
		mSceneMgr->getRootSceneNode()->pitch(-mRotScale);

	if(mKeyboard->isKeyDown(KC_RIGHT))
		mSceneMgr->getRootSceneNode()->yaw(-mRotScale);
...
#define min(a,b)            (((a) < (b)) ? (a) : (b))
	...
	float halfWidth = (float)arg.state.width ;
	float halfHeight = (float)arg.state.height;
	mMouseX = min(mMouseX, halfWidth);
	mMouseY = min(mMouseY, halfHeight);

	mMouseX = max(mMouseX, 0);
	mMouseY = max(mMouseY, 0);


	//float halfWidth = (float)arg.state.width / 2.0f;
	//float halfHeight = (float)arg.state.height / 2.0f;
	//mMouseX = min(mMouseX, halfWidth);
	//mMouseY = min(mMouseY, halfHeight);
	//mMouseX = max(mMouseX, -halfWidth);
	//mMouseY = max(mMouseY, -halfHeight);
